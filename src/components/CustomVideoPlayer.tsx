...
const getBackupUrls = (url: string, channelId: string): string[] => {
  ...
  // If URL is archive.org download format, add serve format as immediate primary fallback
  if (urlLower.includes('archive.org/download/')) {
    const serveUrl = url.replace(/archive\.org\/download\//gi, 'archive.org/serve/');
    if (serveUrl !== url) {
      backups.push(serveUrl);
    }
  } else if (urlLower.includes('archive.org/serve/')) {
    const downloadUrl = url.replace(/archive\.org\/serve\//gi, 'archive.org/download/');
    if (downloadUrl !== url) {
      backups.push(downloadUrl);
    }
  }
  ...
  if (urlLower.endsWith('.mpg')) {
    ...
    backups.push(url.replace(/archive\.org\/download\//gi, 'archive.org/serve/').replace(/\.mpg$/i, '_512kb.mp4'));
  }

  // Primary CORS-safe public live HLS stream (Al Jazeera English is famously highly compliant)
  const ajeLive = 'https://live-als-hls-web-aje.getaj.net/AJE/index.m3u8';
  // Reliable Mux Player capabilities test stream
  const muxTestStream = 'https://test-streams.mux.dev/x36xhzz/x36xhzz.m3u8';
  // Reliable high-bandwidth public domain MP4 video clips hosted on Google Storage (guaranteed open CORS)
  const gcsTearsOfSteel = 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/TearsOfSteel.mp4';
  const gcsSintel = 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/Sintel.mp4';

  if (idLower.includes('rt') || idLower.includes('203') || urlLower.includes('rt_eng') || urlLower.includes('rt-eng')) {
    backups.push(ajeLive);
    backups.push(muxTestStream);
    backups.push(gcsTearsOfSteel);
    backups.push(gcsSintel);
  } else if (idLower.includes('dw') || idLower.includes('204') || urlLower.includes('dwamdstream')) {
    backups.push(ajeLive);
    backups.push(muxTestStream);
    backups.push(gcsSintel);
    backups.push(gcsTearsOfSteel);
  } else if (idLower.includes('cnn') || idLower.includes('201') || urlLower.includes('cnn')) {
    backups.push(ajeLive);
    backups.push(muxTestStream);
    backups.push(gcsTearsOfSteel);
    backups.push(gcsSintel);
  } else if (idLower.includes('fox') || idLower.includes('202') || urlLower.includes('fox')) {
    backups.push(ajeLive);
    backups.push(muxTestStream);
    backups.push(gcsSintel);
    backups.push(gcsTearsOfSteel);
  } else if (idLower.includes('bbc') || idLower.includes('205') || urlLower.includes('skynews') || urlLower.includes('sky-news')) {
    backups.push(ajeLive);
    backups.push(muxTestStream);
    backups.push(gcsTearsOfSteel);
    backups.push(gcsSintel);
  } else if (idLower.includes('newsreels') || idLower.includes('news-') || idLower.includes('westerns') || idLower.includes('adventure') || urlLower.includes('archive.org')) {
    // Vintage media on Archive.org regularly suffers from strict referrer policies, geo-fencing, or throttling
    backups.push(gcsTearsOfSteel);
    backups.push(gcsSintel);
    backups.push(gcsTearsOfSteel);
    backups.push(ajeLive);
  ...
  // Ultimate fail-safe backup pool
  backups.push(ajeLive);
  backups.push(muxTestStream);
  backups.push(gcsSintel);
  backups.push(gcsTearsOfSteel);
...
const resetVideoElement = (videoEl: HTMLVideoElement | null) => {
  ...
  try {
    ...
    videoEl.load(); // Forces WebKit/Blink to release decoder buffer
  } catch (err) {
...
export function CustomVideoPlayer({
  ...
  const triggerFallback = async () => {
    ...
    const pool = scrapedBackupIdentifiers.length > 0 
      ...
      : [
          "s-01e-02.-point-blank",
          "s-01.-e-17-ella-west.ia",
          "rawhide-3-x-30-incident-of-the-wager-on-payday",
          "death-valley-days-s-01-e-04-the-lost-pegleg-mine",
          "the-lone-ranger-s01e01-enter-the-lone-ranger",
          "bonanza-s01e01-a-rose-for-lotta",
          "cisco-kid-s01e01-boomerang",
          "the-rifleman-s01e01-the-sharp-shooter",
          "wagon-train-s01e01-the-willy-moran-story"
        ];

    // Find one that we haven't failed on yet
    const remainingIds = pool.filter(id => !failedUrlsRef.current.includes(id));
    if (remainingIds.length === 0) {
      // Fallback to Al Jazeera English or Mux test streams if all archive links fail
      const lastResorts = [
        'https://live-als-hls-web-aje.getaj.net/AJE/index.m3u8',
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Properly release hardware decoder resources on both player instances to prevent DECODER STALL
  ...
  return (
    <div
      ...
      className={
        isCinemaBackdrop
          ? "fixed inset-0 w-screen h-screen z-1 bg-black overflow-hidden select-none flex flex-col justify-between"
          : "relative z-10 aspect-[4/3] max-w-5xl mx-auto w-full bg-black overflow-hidden border border-white/10 rounded-xl shadow-2xl group flex flex-col justify-between"
      }
    >
